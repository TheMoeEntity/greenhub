  ...
  const dataTypes = [
    {
      ...
      text: "Energy Consumption and Efficiency",
      isActive: false,
    },
    ...
    },
    {
      ...
      text: "Biodiversity and Green Spaces",
      isActive: false,
    },
  ];
      ...
      <div className="flex mx-auto max-w-screen-2xl flex-col gap-10 py-16 px-5 md:px-8">
        <div className="mx-auto text-center w-fit flex flex-col gap-8">
          <h2 className="text-3xl">
            The UNILAG GREENHUB sustainability metrics
          </h2>
          <p>
            we provide campus data on different efforts aimed at positive impact
            on climate change.
          </p>
        </div>
        <div className="flex md:p-8 flex-col gap-10 gap-y-16 flex-wrap md:flex-row justify-center">
          {dataState.map((item, index) => (
            <div
              key={item.text}
              className="flex-1 md:flex-[45%] lg:flex-[0.3] lg:basis-[30%] flex flex-col gap-5"
            >
              ...
              <p className="text-sm leading-loose text-justify">
                The Circular Economic Bazaar provides knowledge and insight into
                the recycling value-chains and products. It promotes
                interactions between the university and the circular community
                for mutual learning.
              </p>
              <button
                onClick={() => accordionAction(item.text)}
                className="bg-[#71cd6c] rounded-md text-white px-12 py-2 w-fit"
              >
                View
              </button>
              <div
                className={`transition-all z-[999] border-b-[1px] duration-300 overflow-x-hidden ${item.isActive ? "max-h-auto py-5" : "max-h-0"}`}
              >
                {index == 2 ? (
                  <a href="/waste.pdf" target="_blank">
                    View metric
                  </a>
                ) : (
          ...
          <i
            aria-hidden
            className="flex-1 md:flex-[45%] lg:flex-[0.3] lg:basis-[30%] "
          ></i>
          <i
            aria-hidden
            className="flex-1 md:flex-[45%] lg:flex-[0.3] lg:basis-[30%] "
